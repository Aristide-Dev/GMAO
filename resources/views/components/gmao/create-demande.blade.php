...
<button class="add-new btn btn-primary waves-effect waves-light" data-bs-toggle="offcanvas"
    data-bs-target="#offcanvasAddDemande" align="right">
...
<div class="offcanvas offcanvas-end {{ $show }}" tabindex="-1" id="offcanvasAddDemande"
    aria-labelledby="offcanvasAddDemandeLabel">
    ...
    <div class="flex-grow-0 pt-0 mx-0 offcanvas-body h-100">
        <form class="pt-0 add-new-demande" id="addNewDemandeForm" enctype="multipart/form-data" method="POST"
            action="{{ route($routeName) }}">
            @csrf
            <div class="mb-3">
                <label class="form-label" for="site">site</label>
                <select id="site" name="site" class="select2 form-select form-select-lg" data-allow-clear="true"
            ...
            @if ($action === 'admin')
                ...
                <div class="mb-3">
                    ...
                    <select id="demandeur" name="demandeur" class="select2 form-select form-select-lg"
                        data-allow-clear="true" data-placeholder="--- CHOISIR UN DEMANDEUR ---">
                        <option value="">--CHOISIR--</option>
                        @foreach ($demandeurs as $demandeur)
                            <option value="{{ $demandeur->id }}">{{ $demandeur->first_name }}
                                {{ $demandeur->last_name }}</option>
                        @endforeach
                    </select>
            ...
            <div id="gmao_file_loder_contennaire">
                <label class="gmao_file_loder">
                    <div class="gmao_file_loder-inner">
                        ...
                        <input id="file-input" name="photo_demande_intervention" type="file"
                            accept=".jpeg,.jpg,.png" />
                    </div>
